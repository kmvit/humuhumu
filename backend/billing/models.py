...
class Subscription(models.Model):
    ...
    plan = models.CharField(
        ...
        default=SiteSettings.Plan.COUNTER,
    )
    ...
    # Генерация фото блюд — единственное, за что мы платим по факту
    # обращения (OpenRouter), поэтому её рубильник и счётчик живут здесь,
    # рядом с деньгами, а не в настройках заведения: ставит их «Падача», а
    # не кафе. Тарифом это не управляется — сетка у нас по формату зала,
    # а расход по картинкам у всех разный.
    images_enabled = models.BooleanField(
        "Генерация фото меню", default=True,
        help_text="Выключить — раздел пропадёт у кафе из каталога",
    )
    image_limit = models.PositiveIntegerField(
        "Генераций в месяц", default=100,
        help_text="Сколько картинок точка может нарисовать за месяц. "
                  "Разовую добавку удобнее давать в «Лимитах генераций фото».",
    )
    notes = models.TextField("Заметки", blank=True)
    ...
    def save(self, *args, **kwargs):
        """Держим настройки заведения в согласии с подпиской.

        Тариф читают в обоих местах (панель владельца, Django-админка), и
        расхождение выглядело бы как ошибка: «оплачен Зал, а станций нет».
        Вместе с тарифом едет формат обслуживания: он и есть то, за что
        берут разную цену, поэтому назначаться должен здесь, а не в
        настройках заведения. Источник правды — подписка.
        """
        super().save(*args, **kwargs)
        from core.models import SiteSettings
        from core.plans import apply_plan
        from core.tenancy import organization_context

        with organization_context(self.organization):
            apply_plan(SiteSettings.load(), self.plan)
