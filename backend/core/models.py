...
class SiteSettings(TenantMixin):
    ...
    # Тариф — то, что продаёт лэндинг «Падачи». Ступеней две, и делит их не
    # набор функций, а формат заведения: «Стойка» дешевле, потому что точка
    # без зала меньше, а не потому, что ей чего-то не дают. Единственная
    # разница в возможностях — экраны кухни и бара: у стойки один человек
    # собирает заказ целиком, делить его между станциями не по чему.
    # Что входит в тариф — один словарь в core/plans.py.
    plan = models.CharField(
        "Тариф",
        max_length=8,
        choices=Plan.choices,
        default=Plan.COUNTER,
        help_text=(
            "«Стойка» — точка без зала; «Зал» — с посадкой и официантами. "
            "Функционал полный на обоих, кроме экранов кухни и бара."
        ),
    )
    ...
    # Формат едет за тарифом: цена зависит от него, поэтому выбирать его
    # заведение не может — ни в панели, ни запросом в API. Ставит подписка
    # (billing.Subscription.save) или сверка лицензии, оба через
    # core.plans.mode_for_plan.
    ...
    # Предоплата на стойке. У окна выдачи никто не отвечает за гостя,
    # который назаказывал и не пришёл: бар успел всё приготовить, и это
    # чистый убыток. В зале иначе — там за стол отвечает официант,
    # поэтому настройка действует только в формате «Стойка».
    prepay_required = models.BooleanField(
        "Готовить только после оплаты", default=True,
        help_text=(
            "Формат «Стойка»: заказ гостя ждёт оплаты и не уходит на кухню, "
            "пока не оплачен. Работает, только если включена оплата картой — "
            "иначе гостю нечем платить, и заказ принимается как раньше."
        ),
    )
    ...
    # ── Технический перерыв ─────────────────────────────────────────────
    # Выключатель гостевого заказа: кофемашина встала, кухня не успевает,
    # закрылись раньше. Меню при этом видно — гость должен понимать, куда
    # попал, и прочитать причину, а не упереться в пустую страницу.
    # Персонала не касается: официант оформляет заказ как обычно, а уже
    # принятый заказ гость дооплачивает и отслеживает.
    ordering_paused = models.BooleanField(
        "Технический перерыв", default=False,
        help_text="Гость видит меню, но отправить заказ не может",
    )
    ordering_pause_note = models.CharField(
        "Что показать гостю", max_length=200, blank=True,
        help_text=(
            "Напр. «Кофемашина на профилактике, ждём к 14:00». "
