...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def track(self, request):
        ...
        if token:
            order = (
                Order.objects.prefetch_related("items__variant__product__category", "items__modifiers")
    ...
    def move_items(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            for pk_ in (order.pk, target.pk):
                o = Order.objects.prefetch_related("items__variant__product__category", "items__modifiers").get(pk=pk_)
                o.recalc_total()
                o.save(update_fields=["total"])
                self._sync_times(o, Category.Station.KITCHEN)
                self._sync_times(o, Category.Station.BAR)
        order = Order.objects.prefetch_related("items__variant__product__category", "items__modifiers").get(pk=order.pk)
        return Response(OrderSerializer(order).data)
    ...
    def _reload_and_respond(self, pk, *stations):
        order = Order.objects.prefetch_related("items__variant__product__category", "items__modifiers").get(pk=pk)
        for st in stations:
