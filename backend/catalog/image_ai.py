...
#: Правила кадра, верные для ЛЮБОЙ посуды. Про материал здесь нарочно
#: ничего нет: «стакан непрозрачный» — правда для бумажного и враньё для
#: стеклянного, и общий промпт такого знать не может. Материал живёт в
#: подписи к образцу (DishwareSample.note) — сказан один раз и едет со
#: своей посудой. Здесь — только то, что не зависит от неё:
_FRAMING = (
    "Блюдо занимает большую часть кадра, снято близко. Напиток или еда "
    "видны ровно настолько, насколько позволяет эта посуда: сквозь "
    "непрозрачные стенки содержимое не просвечивает, сквозь прозрачные — "
    "видно."
)

#: Что не должно попасть на фото меню ни при каких настройках.
#: Прямой запрет «не придумывай надписей и эмблем» отсюда УБРАН, и это
#: проверено живьём: с ним модель нарисовала логотип крупнее и наглее,
#: чем без него. Так устроены картиночные модели — названное в запрете
#: слово тянет за собой саму вещь. Выдуманные эмблемы лечатся не словами,
#: а приложенным образцом: там, где фото посуды есть, модель копирует
#: печать с него и ничего не сочиняет. А если образца нет, генерация до
#: модели и не доходит — см. run_generation.
_FORBIDDEN = (
    "Без посторонних надписей, ценников, чужих логотипов и водяных знаков; "
    "то, что напечатано на нашей посуде, оставь как есть. Без людей и рук "
    "в кадре. Естественные цвета, реалистичная порция — столько, сколько "
    "правда помещается в эту посуду."
)
...
def run_generation(generation) -> None:
    ...
    try:
        ...
        if dishware and not refs:
            ...
            # фото», модель на пустом месте сочинит посуду с выдуманным
            # логотипом, и это уедет в меню как наше. Лучше честная ошибка —
            # и до модели не ходим, обращение не оплачиваем.
            raise LLMError(
                "Не удалось прочитать фото посуды — загрузите образец заново."
            )
