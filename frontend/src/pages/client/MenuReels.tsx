...
export default function MenuReels() {
  ...
  // страницы пейджера: категории с позициями + «Хиты» (топ по лайкам) в конце.
  ...
  const pages = useMemo(() => {
    ...
    const topPage = top.length
      ...
      : [];
    // «Хиты» последними: гость открывает ленту на меню, а не на витрине
    // лайков — и не пролистывает её каждый раз, чтобы дойти до еды
    return [...catPages, ...topPage];
  }, [categories, products]);
